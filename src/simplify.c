...
static void clear_phi(struct dmr_C *C, struct instruction *insn)
{
	pseudo_t phi;
...
void kill_instruction(struct dmr_C *C, struct instruction *insn)
{
	if (!insn || !insn->bb)
		return;
  ...
  case OP_OR_BOOL:
	...
	case OP_SYMADDR:
		insn->bb = NULL;
		C->L->repeat_phase |= REPEAT_CSE | REPEAT_SYMBOL_CLEANUP;
		return;

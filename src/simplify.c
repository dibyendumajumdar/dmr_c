...
static int simplify_asr(struct dmr_C *C, struct instruction *insn, pseudo_t pseudo, long long value)
{
	...
	if (value >= size) {
		...
		return replace_with_pseudo(C, insn, value_pseudo(C, 0, insn->type));
	}
	if (!value)
...
static int simplify_constant_rightside(struct dmr_C *C, struct instruction *insn)
{
	...
	case OP_SUB:
		if (value) {
			insn->opcode = OP_ADD;
			insn->src2 = value_pseudo(C, -value, insn->src2->sym);
			return REPEAT_CSE;
		}
	/* Fall through */
	case OP_ADD:
...
static int simplify_constant_binop(struct dmr_C *C, struct instruction *insn)
{
	...
	replace_with_pseudo(C, insn, value_pseudo(C, res, insn->type));
	return REPEAT_CSE;
}
...
static int simplify_binop_same_args(struct dmr_C *C, struct instruction *insn, pseudo_t arg)
{
	...
	case OP_SET_BE: case OP_SET_AE:
		...
		return replace_with_pseudo(C, insn, value_pseudo(C, 1, insn->type));
	...
	case OP_OR_BOOL:
		...
		insn->src2 = value_pseudo(C, 0, pseudo_type(C, insn->src1));
...
static int simplify_cast(struct dmr_C *C, struct instruction *insn)
{
	...
	if (constant(src)) {
		...
		src = value_pseudo(C, val, insn->type);
		goto simplify;
	}

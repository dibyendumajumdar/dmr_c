...
def execute_test(c_file):
    ...
    try:
        ...
        result = subprocess.run([Sparse_Executable, "-O2", c_file], universal_newlines=True,
...
def run_a_test(filename, results_directory):
    ...
    """
    (status, actual_output) = execute_test(filename)
    if not status:
...
Sparse_Executable = shutil.which('sparse-omrjit', os.X_OK)
if not Sparse_Executable:
    print('sparse-omrjit must be on the PATH')
    exit(1)
...
# 8cc, picoc, set1 etc use features that are unsupported
ignored_list = ['unsupported', 'parsetree', 'sqlite', 'minilua', 'picoc', 'set1', '8cc', 'nano']

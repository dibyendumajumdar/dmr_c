...
* The LLVM backend takes the SSA IR generated by the linearizer in sparse and converts these to LLVM IR instructions.
* You can view the linearizer output by running the linearize command.
* The Sparse SSA IR is lower level than LLVM IR - in particular it has no concept of types other than primitive int, floating and pinter types. The generated LLVM IR therefore relies upon casting the values where required to types expectey by LLVM.

#### Limitations

* The main limitation is that complex initializers do not work yet - only strings and scalar variables can be initialized. The front-ned handles these - the limitation is in the sparse-llvm backend.
* There is no support for computed gotos, slices and ranges yet.
* The fromt-end parser and pre-processor knows about many Linux constructs hence it can process C header files on Linux. However, it doesn't know about Windows or Mac OSX features. As typically the vendor supplied header files have many platform specific extensions, unfortunately this means that you cannot process vendor supplied header files on these platforms.
